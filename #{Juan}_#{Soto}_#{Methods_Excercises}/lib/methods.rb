...
def last_n_digits(num, n)
  num_string = num.to_s
  return num if n >= num_string.length
  return num % 10 if num_string.length == 1
  until num_string.length == n
      num_string.slice!(0)
  end
  num_string.to_i
end
...
def dec_remainder_of_two_floats(f_dividend, f_divisor)
  int = (f_dividend.to_f / f_divisor.to_f).floor
  dec = (f_dividend.to_f / f_divisor.to_f)
  answer = dec - int
end
...
def dec_remainder_of_two_integers(i_dividend, i_divisor)
  answer = dec_remainder_of_two_floats(i_dividend , i_divisor)
end
...
def int_remainder_without_modulo(i_dividend, i_divisor)
  return i_dividend if i_divisor > i_dividend
  answer = (i_dividend.to_f / i_divisor.to_f).floor
end
