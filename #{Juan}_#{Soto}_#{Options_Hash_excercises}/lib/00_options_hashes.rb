# Options Hashes
#
# Write a method `transmogrify` that takes a `String`. This method should
# take optional parameters `:times`, `:upcase`, and `:reverse`. Hard-code
# reasonable defaults in a `defaults` hash defined in the `transmogrify`
# method. Use `Hash#merge` to combine the defaults with any optional
# parameters passed by the user. Do not modify the incoming options
# hash. For example:
#
# ```ruby
# transmogrify("Hello")                                    #=> "Hello"
# transmogrify("Hello", :times => 3)                       #=> "HelloHelloHello"
# transmogrify("Hello", :upcase => true)                   #=> "HELLO"
# transmogrify("Hello", :upcase => true, :reverse => true) #=> "OLLEH"
#
# options = {}
# transmogrify("hello", options)
# # options shouldn't change.
# ```
options = {
  times: 0,
  upcase: false,
  reverse: false
}
def transmogrify(string,options = nil)
  answer_str = " "
  split_str = string.split(" ")
  if options != nil
    if options[:times] != 0
      (options[:times]).times do |el|
        if options[:upcase] == true && options[:reverse] == true
          answer_str +=  split_str[0].upcase.reverse + " "
        elsif options[:upcase] == false && options[:reverse] == true
          answer_str += split_str[0].reverse + " "
        elsif options[:upcase] == true && options[:reverse] == false
          answer_str +=  split_str[0].upcase + " "
        else
          answer_str += split_str[0] + " "
        end
      end
    elsif options[:upcase] == true && options[:reverse] == true
      answer_str =  split_str[0].upcase.reverse + " "
    else
      answer_str =  split_str[0].reverse + " "
    end
  return answer_str.split.unshift("").join(" ")
  else
  string
  end
end
# transmogrify("string",options = {
#   times: 1,
#   upcase: false,
#   reverse: true
# })
