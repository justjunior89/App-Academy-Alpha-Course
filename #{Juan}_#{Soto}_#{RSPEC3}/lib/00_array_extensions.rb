...
class Array
  def two_sum
    return [] if self.all?{|number| number > 0 }
    idx = 0
    answer_arr = []
    while idx <= self.length - 1
      current_number = self[idx]
      idx2 = idx + 1
      while idx2 <= self.length - 1
        second_number = self[idx2]
          if current_number + second_number == 0
            answer_arr << [idx,idx2]
          elsif current_number == 0 && second_number == 0
            answer_arr << [idx,idx2]
            break
          end
        idx2 += 1
...
class Array
  def median
    ...
    if sort.length.odd?
      ...
      (sort[sort.length / 2] + sort[(sort.length / 2) - 1]) / 2.0
    end
  end
end
...
class Array
  def my_transpose
    matrix_dimensions = self.first.length
    new_matrix = Array.new(matrix_dimensions){Array.new(matrix_dimensions)}
    matrix_dimensions.times do |x|
      matrix_dimensions.times do |y|
